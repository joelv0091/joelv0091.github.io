...
function defaultComapanyConfiguration(companyInput)
{
	...
	if( childObj != undefined && childObj != null  )
		...
		{
		console.log("=====================================");
		console.log("NO CONFIGURATION AVAILABLE FOR STREAM");
		console.log("=====================================");
		}
...
function defaultProfileConfiguration(p)
{
	...
	if( obj !=undefined && obj !=null  )
	{
	   ...
	   LiveStreamObject.connect.server.stream.ip =obj.profileDetails.vmsIp;
		...
		LiveStreamObject.connect.server.stream.project =(obj.profileDetails.vmsProjectName!=undefined &&  obj.profileDetails.vmsProjectName!=null)? obj.profileDetails.vmsProjectName:(obj.profileDetails.vmsVersion==1.8)?"itms-apiserver":(obj.profileDetails.vmsVersion==2.1)?"REST/":"i-apiserver";
...
function ValidateConfigData()
{
	...
	if(LiveStreamObject.connect.server.stream.version==2.1 && LiveStreamObject.connect.server.stream.serverId == undefined)return false;	
	console.log("Serverid");
	if(LiveStreamObject.connect.server.stream.version==2.1 && LiveStreamObject.connect.server.stream.sslport == undefined)return false;
	console.log("sslport");
	if(LiveStreamObject.connect.server.stream.version==2.1 && LiveStreamObject.connect.server.stream.username == undefined)return false;
	if(LiveStreamObject.connect.server.stream.version==2.1 && LiveStreamObject.connect.server.stream.password == undefined)return false;
