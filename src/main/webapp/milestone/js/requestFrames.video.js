...
//		if(fjdi == true)console.log("fimageElement.src----",imageElement.src);
		...
		if(fjdi == true)
			console.log("frame @",frame.timestamp.getTime(),"---User input-- ",LiveStreamObject.connect.continous.date.end);
		
		
		updateInProgressBar(frame.timestamp.getTime() ,LiveStreamObject.connect.continous.date.start, LiveStreamObject.connect.continous.date.end);
		if ( (frame.timestamp.getTime()) > LiveStreamObject.connect.continous.date.end ) {
        	//if(fjdi == true)
			updateInProgressBar(LiveStreamObject.connect.continous.date.end ,LiveStreamObject.connect.continous.date.start, LiveStreamObject.connect.continous.date.end);
        		console.log("Stop Limit -> ",LiveStreamObject.connect.continous.date.end,"-----STOPED STREAM reached end ");
        		console.log("-----STOPED frame date ->",frame.timestamp,"@ frame timestamb ->",frame.timestamp.getTime());
