 ...
 */
var sendCommandRequest = function(message, callback, requestIdentifier) {
	
	var ajaxRequest = new XMLHttpRequest();
	try{
	//ajaxRequest.open('POST', '/XProtectMobile/Communication');
	ajaxRequest.open('POST', 'http://'+LiveStreamObject.connect.server.parent.ip+':'+LiveStreamObject.connect.server.parent.port+'/XProtectMobile/Communication');
//	ajaxRequest.open('POST', 'http://192.168.1.206:9030/XProtectMobile/Communication');
	ajaxRequest.setRequestHeader('Content-Type', 'text/xml; charset=utf-8');
	if(fjdi == true)console.log("CameraId-"+LiveStreamObject.camera.Id);
	if(fjdi == true)console.log("message-"+message);
	if(fjdi == true)console.trace("CameraId-"+LiveStreamObject.camera.Id);
	...
	ajaxRequest.onreadystatechange = function() {
		...
		if (ajaxRequest.readyState == 4) {
			if (ajaxRequest.status == 200) {
				...
				if(fjdi == true)console.log('Response received.', ajaxRequest.responseText);
				...
				if (isResponseOk(response)) {
					...
					{
						callbackManager(requestIdentifier,response);
					}
				}
			} else {
				...
				/// new changes
					...
					if(requestIdentifier !=  undefined && requestIdentifier==5 )
					{
						callbackManager(requestIdentifier,response);
					}
					...
					document.getElementById("livestatus").innerHTML = "Server Unavailable";
			...
			//console.trace();
			 ...
			 printScreen("Something went wrong",2000);
			
			if(fjdi == true)
				{console.trace();console.log('Something went wrong with the request2.', ajaxRequest.readyState);}
			}
	};
	
	if(fjdi == true)console.log('Sending XML message to the Mobile Server.', message);
	ajaxRequest.send(message);
	
}catch(e){
	document.getElementById("livestatus").innerHTML = "Something went wrong.....";
	if(fjdi == true)console.log('Server Unavailable');
	}
};
...
PlayBackVideoAPI.generateXMLMessage = function(options) {
...
var isResponseOk = function(response) {
	...
	if (result == 'OK') {
		...
		if(fjdi == true)console.log('Command ' + commandName + ' finished successfully.');
		return true;
	} else {
		
		if(fjdi == true)console.log('Command ' + commandName + ' finished with an error.');
		...
		if (response.querySelector('Command ErrorString')) {
			...
			console.log("--ERRROORRR-=----------------" ,response.querySelector('Command ErrorCode').firstChild.textContent);
