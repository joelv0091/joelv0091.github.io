		...
		{	
			...
			var win = window.open("", "Title", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=525,height=525,top="+(screen.height-400)+",left="+(screen.width-840));
			win.document.body.innerHTML = "<a href='data:image/png;base64,"+data+"' download='Camera_Incident_Screenshot'>"+
"<img src='data:image/png;base64,"+data+"' alt='Camera'  style='width:500px;height:500px;' /><a>";
		}
		...
		//	xhrFields: { withCredentials: true },
			success: function(output, textStatus, jqXHR) {
				...
				var win = window.open("http://"+ESBpublicURL+"/CameraCookieIntegration/"+output.data+"/"+LiveStreamObject.connect.server.stream.serverId+"/"+LiveStreamObject.camera.id, "Image");
				 //4000
		...
		$.ajax({
			type: "POST",
		    url: "/TrinityLiveStream/api/videonetcs/unified/getCurrentSnap",
			data: requestBody,
			contentType: 'application/json',
		//	xhrFields: { withCredentials: true },
			success: function(output, textStatus, jqXHR) {
