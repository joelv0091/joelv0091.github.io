...
console.log("live---"+location.protocol);
 	...
 	return (situvtn==0 ) ? url+LiveStreamObject.connect.server.stream.project:(situvtn==1 ) ? url+LiveStreamObject.connect.server.stream.project+LiveStreamObject.connect.server.stream.serverId+"/startlive": (situvtn==2 ) ? url+LiveStreamObject.connect.server.stream.project+LiveStreamObject.connect.server.stream.serverId+"/live/keepalive" : (situvtn==3 ) ? url.substring(0, url.length - 1):   (situvtn==4 ) ? url+LiveStreamObject.connect.server.stream.project+LiveStreamObject.connect.server.stream.serverId+"/ptz/channel" :(situvtn==5 ) ? url+LiveStreamObject.connect.server.stream.project+"user/login": (situvtn==6 ) ?  url+LiveStreamObject.connect.server.stream.project+LiveStreamObject.connect.server.stream.serverId+"/gotopreset": (situvtn==7 ) ? url+LiveStreamObject.connect.server.stream.project+LiveStreamObject.connect.server.stream.serverId+"/channel":"NULL";
 }