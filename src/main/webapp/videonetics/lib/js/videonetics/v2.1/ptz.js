...
/**
 * Up, Down, Left, Right, UpLeft, UpRight, DownLeft, DownRight, ZoomIn, ZoomOut,
 * Home.
 * 
 */
var ptzaction_v2p1 = function(actn, cam) {
	// alert(actn);
	ptzRequest_v2p1(actn, cam);
	setTimeout(function() {
		ptzRequest_v2p1(actn, cam);
	}, 1000);
	setTimeout(function() {
...
var ptzRequest_v2p1 = function(actn, cam) {
	// alert(actn);
	var dataobject = {
	...
	$.ajax({
		...
		/*
		 * }).done(function(data, textStatus, ajax) { console.log("ptzaction
		 * -action :"+actn+" on -cam"+LiveStreamObject.camera.id);
		 * 
		...
		success : function(output, textStatus, jqXHR) {
			...
			// alert("Sucess--");
			if (data.status && data.code && data.status == 200
					&& data.code == 2000) {
				console.log(new Date(), ": PTZ Action Sucess");
			} else {
				console.log(new Date(), ": FAILED - LIVE EXTENDED : ",
						data.description);
				console.log("PARTNER STATUS CODE : ", data.code,
						"PARTNER STATUS status : ", data.status);
				// startalive_v2p1();
			}
		...
		error : function(jqXHR, textStatus) {
			...
			LiveStreamObject.error.log.push([ new Date(), " : Error @ PTZ ::",
					"jqXHR:", jqXHR, "textStatus", textStatus, "URL:",
