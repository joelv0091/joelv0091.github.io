...
function intialiseKeepLive_v2p0()
{
	console.log(new Date(),": intialiseKeepLive_v2p0");

	 LiveStreamObject.timer.object= setInterval(function(){
		 KeepLiveRequest_v2p0();
 }, 60000 * LiveStreamObject.timer.interval);

}

function KeepLiveRequest_v2p0()
{
	LiveStreamObject.camera.videonetics.keepLive = urlcreator_v2p0(2);
	console.log("keepalive.js  started after 4minutes--KeepLiveRequest_v2p0",LiveStreamObject.camera.videonetics.keepLive+"/"+LiveStreamObject.camera.videonetics.sessionId);
